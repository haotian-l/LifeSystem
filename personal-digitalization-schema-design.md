...
所有可留存事实都默认进入 LifeSystem。SQLite 是结构化事实的主账本；聊天回复、临时记忆、Markdown 文件、Notion 或其他外部系统都不能替代 LifeSystem 里的事实记录。
...
1. 数据库：保存可查询、可统计、可关联的结构化事实，是唯一默认主账本。
2. 文件系统：保存照片、视频、音频、导出文件等大对象，不保存事实权威状态。
...
数据库不直接保存大文件，只保存文件路径、校验值和元信息。文件系统只作为大对象和导出结果的存放层，不作为结构化事实的默认账本。文件系统建议按用途和时间组织。
...
- SQLite 是结构化事实的主账本。
- 受控标签集合以 SQLite `tags` 表为准；如导出 `config/tags.json`，它只是快照。
- 人物索引、目标、任务、习惯、日程、复盘和提醒的结构化状态都以 SQLite 为准。
- `assets/` 保存原始照片、截图、语音、文档。
- `reports/` 可保存报告导出文本，但报告事实、周期和状态仍以 SQLite 为准。
...
- 日视图中卡片的垂直位置必须对应真实 `start_at`；排版可以压缩文字，但不能为了避免重叠而把事件推到错误小时。
- 周视图：展示一周时间分布。
- 月视图：展示每天的主要活动和高光。
...
如果以后要同步到 Notion、日历、相册、Markdown 文件或其他外部系统，可以增加 Sync Link。外部同步必须是显式动作，不能成为默认写入路径。
